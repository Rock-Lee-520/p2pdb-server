...
func (e *Engine) QueryNodeWithBindings(
	...
	debug.Dump("QueryNodeWithBindings====== start")
	if parsed == nil {
		parsed, err = parse.Parse(ctx, query)
		if err != nil {
			return nil, nil, err
	...
	debug.Dump("QueryNodeWithBindings======  -1")
	transactionDatabase, err := e.beginTransaction(ctx, parsed)
	if err != nil {
		return nil, nil, err
	}
	debug.Dump("QueryNodeWithBindings======  -2")
	if len(bindings) > 0 {
	...
	debug.Dump("QueryNodeWithBindings======  -3")
	analyzed, err = e.Analyzer.Analyze(ctx, parsed, nil)
	if err != nil {
		debug.Dump(err)
		return nil, nil, err
	}
	debug.Dump("QueryNodeWithBindings======  -4")
	iter, err = analyzed.RowIter(ctx, nil)
	if err != nil {
		return nil, nil, err
	}
	debug.Dump("QueryNodeWithBindings======  -5")
	autoCommit, err := isSessionAutocommit(ctx)
	if err != nil {
		return nil, nil, err
	}
	debug.Dump("QueryNodeWithBindings======  -6")
	if autoCommit {
		iter = transactionCommittingIter{iter, transactionDatabase}
	}
	//debug.Dump("QueryNodeWithBindings====== start")
	debug.Dump("QueryNodeWithBindings====== end")
