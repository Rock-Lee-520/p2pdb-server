...
func (h *Handler) doQuery(
	...
	defer timer.Stop()
	ctx.GetLogger().Debugf("echo  sql start==========")
	...
	eg.Go(func() error {
		...
		for {
			...
			//ctx.GetLogger().Debugf("echo  r is==========", r)
			...
			case row, ok := <-rowChan:
				...
				//	ctx.GetLogger().Debugf("echo  sql middel==========", outputRow)
				if err != nil {
					return err
				}
	...
	ctx.GetLogger().Debugf("echo  sql end==========")
	err = eg.Wait()
	if err != nil {
		ctx.GetLogger().WithError(err).Warn("error running query")
		return remainder, err
